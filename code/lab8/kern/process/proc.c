...
alloc_proc(void) {
	...
	if (proc != NULL ) {
		...
		//LAB6 2009011419 : (update LAB5 steps)
		/*
		...
		proc->lab6_stride = (1 << 20);
		proc->lab6_priority = 1;

		//LAB8:EXERCISE2 2009011419
		//    HINT:need add some code to init fs in proc_struct, ...
...
static int copy_files(uint32_t clone_flags, struct proc_struct *proc) {
	...
	good_files_struct: files_count_inc(filesp);
	proc->filesp = filesp;
	return 0;

	bad_dup_cleanup_fs: files_destroy(filesp);
	bad_files_struct: return ret;
}
...
static int load_icode(int fd, int argc, char **kargv) {
	 ...
	 *     HINT:how to load the file with handler fd  in to process's memory?
